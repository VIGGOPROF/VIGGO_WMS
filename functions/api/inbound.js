  ...
  try {
    ...
    for (const item of items) {
        ...
        if (factory && !validFactories.has(factory.toLowerCase())) {
            return new Response(JSON.stringify({ error: `La fábrica "${factory}" no está registrada.` }), { status: 400 });
        }
    ...
    if (statements.length === 0) return new Response(JSON.stringify({ error: "No se encontraron datos válidos." }), { status: 400 });

    await db.batch(statements);
    return new Response(JSON.stringify({ status: "success", message: `Ingresaron ${totalQty} unidades.` }), { headers: { "Content-Type": "application/json" } });
